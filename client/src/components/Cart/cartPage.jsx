...
const Cart = () => {
  ...
  /* Traemos del context los productos del carrito y la función clearCart */
  const { cartItems, clearCart, confirmCompra } = useContext(CartContext);
    ...
    <div className={styles.cartContainer}>
      ...
      {cartItems && cartOpen && (
        <div className={styles.cart}>
          ...
          {cartItems.length > 0 && (
            <button
              onClick={() => confirmCompra(cartItems)}
              className={styles.confirmarCompraButton}
            >
              <Link to={'/compraExitosa'}>Confirmar Compra</Link>
